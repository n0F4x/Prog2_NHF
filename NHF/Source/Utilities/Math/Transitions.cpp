	...
	EaseInOut::EaseInOut(Transitionable* object) :
		Transition{ object, [this](int elapsedTime) -> sf::Vector2f {
			...
			if (elapsedTime <= getDurationTime() / 2) {
				progression = {
					_accX / 2.f * static_cast<float>(math::square(elapsedTime)) - _accX / 2.f * static_cast<float>(math::square(getElapsedTime())),
					_accY / 2.f * static_cast<float>(math::square(elapsedTime)) - _accY / 2.f * static_cast<float>(math::square(getElapsedTime()))
				};
			}
			else {
				progression = {
					_accX / 2.f * static_cast<float>(math::square(getDurationTime() - getElapsedTime())) - _accX / 2.f * static_cast<float>(math::square(getDurationTime() - elapsedTime)),
					_accY / 2.f * static_cast<float>(math::square(getDurationTime() - getElapsedTime())) - _accY / 2.f * static_cast<float>(math::square(getDurationTime() - elapsedTime))
				};
			}
			return progression;
		}
		...
		Jump::Jump(Transitionable* object) :
			Transition{ object, [this](int elapsedTime) -> sf::Vector2f {
				float progressionF = _acc / 2.f * static_cast<float>(math::square(elapsedTime)) + _velocity * static_cast<float>(elapsedTime) - _lastProgress;
