...
float BezierEasing::BinarySubdivide(float time, float interval_start, float next_interval_step, float p1_coord, float p2_coord) const {
	...
	do {
		current_t = interval_start + (next_interval_step - interval_start) * 0.5f;
...
float BezierEasing::GetXForTime(float time) {
	...
	float dist = (time - _sample_values[curr_sample]) / (_sample_values[static_cast<unsigned>(curr_sample) + 1u] - _sample_values[curr_sample]);
